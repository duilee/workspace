...
def group_by_centroid(restaurants, centroids):
    ...
    for i in restaurants:
        pairs += [[find_closest(restaurant_location(i), centroids), i]]
...
def find_centroid(cluster):
    ...
    longi = []
    for i in cluster:
        lati += [restaurant_location(i)[0]]
        longi += [restaurant_location(i)[1]]
...
def best_predictor(user, restaurants, feature_fns):
    ...
    for i in feature_fns:
        lst = lst + [find_predictor(user, reviewed, i)[1]]

    for x in range(len(feature_fns)):
        if max(lst) == lst[x]:
            return find_predictor(user, reviewed, feature_fns[x])[0]
...
def rate_all(user, restaurants, feature_fns):
    ...
    for x in restaurants:
        if restaurant_name(x) not in dic:
            dic[restaurant_name(x)] = predictor(x)
