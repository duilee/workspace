...
class T88ble():
    ...
    def with_column(self, label, values):
        ...
        """
        new_rows = []
        new_col = self.column_labels[:]

        new_col += [label]
        index = 0
        for i in self.rows:
            new_rows += [i + [values[index]]]
            index += 1

        new = T88ble(new_rows, new_col)
    ...
    def select(self, labels):
        ...
        for j in labels:
            index = 0
            for i in self.rows:
                if len(new_rows) < (index + 1):
                    new_rows += [[i[self.labels().index(j)]]]
                    index += 1
                else:
                    new_rows[index] += [i[self.labels().index(j)]]
                    index += 1

        new = T88ble(new_rows, labels)
    ...
    def sort(self, label, descending=True):
        ...
        """
        lst = [i[self.labels().index(label)] for i in self.rows]
        new_rows = []

        for j in sorted(lst, reverse = descending):
            for k in self.rows:
                if k[self.labels().index(label)] == j:
                    new_rows += [k]
